...
public class CrfLogLikelihoodFunction<K, G> extends DerivableFunction {
    ...
    /**
     * Constructs the log-likelihood function of the CRF. Note that the whole
     * corpus should be inside the internal memory (the JVM heap). It should not
     * be in disk or data-base.
     *
     * @param corpus A corpus -- a list of sequences of tokens, where each token
     * has a tag (this is a gold-standard corpus: the tags are known).
     * @param crfTags The tags that exist in the given corpus. See
     * {@link CrfTagsBuilder}.
     * @param features The CRF features.
     * @param useRegularization whether to use a regularization factor or not.
     * @param sigmaSquare_inverseRegularizationFactor the \sigma^2 parameter of
    ...
    private CrfModel<K, G> createModel(double[] point) {
        if (point.length != features.getFilteredFeatures().length) {
            throw new CrfException("Number of parameters differs from number of features.");
        }
...
//	}
     ...
     * <B>The whole corpus should reside in the internal memory completely!</B>
     * Otherwise, the run will be very slow.
     */

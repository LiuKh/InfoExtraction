...
import infoextraction.InfoExtract.unstrcturedInfo.function.TwiceDerivableFunction;
import infoextraction.InfoExtract.unstrcturedInfo.utilities.CrfException;

/**
 * Represent "-f(x)" for a given function "f(x)".
 * <BR>
 * This negated function can be used when maximization of a function is needed,
 * but a minimization algorithm is available. Just minimize "-f(x)", and the
 * resulting "x" is the point of the maximum for "f(x)".
 *
 * @author Asher Stern Date: Nov 6, 2014
 *
 */
public class NegatedFunction extends TwiceDerivableFunction {
    ...
    public double value(double[] point) {
        if (function != null) {
            ...
            return -derivableFunction.value(point);
        } else if (twiceDerivableFunction != null) {
            return -twiceDerivableFunction.value(point);
        } else {
    ...
    public double[] gradient(double[] point) {
        if (derivableFunction != null) {
            return negate(derivableFunction.gradient(point));
        } else if (twiceDerivableFunction != null) {
            return negate(twiceDerivableFunction.gradient(point));
        } else {
            throw new CrfException("BUG");
        }
    }
